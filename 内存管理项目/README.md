...
通过本次课程设计，我对操作系统中动态分区存储管理的过程有了更直观的理解。动态分区分配不像固定分区那样提前划定内存块，而是根据作业申请空间的大小动态切割空闲区，这就需要系统持续维护空闲分区表和已分配分区表。

在实现过程中，我认识到 FF 和 BF 算法的差异并不一定在前几次操作中立刻体现出来。只有经过多次申请、释放和空闲区合并后，内存中形成多个大小不同的空闲区，两种算法才更容易表现出不同选择。FF 更强调从低地址开始尽快找到可用分区，BF 更强调选择最小可用分区。

释放内存后的空闲区合并也是本项目的重要部分。如果只把释放出来的分区加入空闲表而不合并，相邻的空闲空间就会被错误地拆成多个小分区，导致后续较大的作业可能分配失败。通过按起始地址排序并检查相邻关系，可以较好地模拟操作系统回收连续内存空间的过程。

图形界面方面，本项目使用 PyQt5 将两种算法左右并排显示，并通过内存分布图、表格和日志同时展示系统状态。这样不仅能看到最终结果，也能看到每一步操作如何影响空闲区和已分配区，对理解动态分区分配过程比较有帮助。

总体来说，本项目把操作系统课程中的连续内存分配、空闲区维护、分区回收和分配算法比较与图形化程序结合起来，加深了我对内存管理部分知识的理解，也提高了我使用 Python 和 PyQt5 开发完整课程设计程序的能力。
